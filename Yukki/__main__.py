...
@app.on_message(filters.command("helppp") & filters.private)
async def helpp_command(_, message):
    text, keyboard = await help_parser(message.from_user.mention)
    await app.send_message(message.chat.id, text, reply_markup=keyboard)


@app.on_message(filters.command("starttt") & filters.private)
        ...
        key = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton(
                        text="🔄 Close", callback_data="closee"
                    ),
                ],
            ]
        )
